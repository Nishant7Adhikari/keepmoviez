...
## 2026-09-23 - Single-pass pace & watch date set in calculateAllStatistics
**Learning:** `calculateAllStatistics` in `js/analysis.js` previously executed 6 separate $O(W)$ loops over `allWatchInstances` (3 `calculatePace` calls + 3 `getNormalizedPaceData` calls) and ran `[...new Set(allWatchInstances.map(wi => wi.date))]` string mapping to compute unique watch dates. Pre-collecting `uniqueWatchDates` during the initial movie loop and consolidating pace watch time totals and histogram bin distributions into a single $O(W)$ pass over `allWatchInstances` eliminated 6 array iterations and array mapping allocations, reducing execution time significantly.
**Action:** When computing multi-window statistics or time-series histogram bins over collection items, compute window totals and bin counts simultaneously in a single pass instead of invoking separate filter/iteration helpers per time window.

## 2026-09-24 - Pre-aggregating watch history into Map & single Date cursor in renderActivityHeatmap
**Learning:** `renderActivityHeatmap` in `js/reporting.js` previously executed `forEach` lookups over `window.movieData` using plain object key checks and instantiated `new Date(startDate)` ~378 times inside nested column/row grid loops (`54 cols x 7 rows`). Pre-aggregating watch history into a `Map` in a single pass and incrementing a single `currentDate` cursor across grid cells reduced execution time by ~1.73x (from 1.81s to 1.04s for 1,000 heatmap renders) while eliminating hundreds of temporary `Date` object allocations per render.
**Action:** Pre-aggregate watch instances into a Map before rendering grid charts, and advance a single Date cursor rather than allocating `new Date(startDate)` inside cell iteration loops.

## 2026-09-25 - Single-pass option collection & DocumentFragment in populateFilterModalOptions
**Learning:** `populateFilterModalOptions` in `js/ui.js` previously executed multiple array `.map()`, `.filter()`, and `Set` operations over `movieData` to build category, country, and language filter options, called `getCountryFullName(opt)` repeatedly in option loops, and appended option elements individually to select elements. Consolidating array mapping into a single $O(N)$ pass over `movieData`, reusing pre-indexed `countryNameMap` lookups, and batching option appends via `DocumentFragment` reduced modal population time from 1.13ms to 0.42ms per call (~2.67x speedup for 5,000 items).
**Action:** Consolidate multi-field array mappings over large collections into a single linear loop, reuse pre-indexed label Maps in option loops, and append DOM children via DocumentFragments to minimize layout reflows.
