...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (menuThemeToggleBtn) {
        ...
        if (themeIcon) themeIcon.className = `fas ${savedTheme === DARK_THEME ? 'fa-moon' : 'fa-sun'} mr-2`;
    }
    ...
    const updateSyncUI = () => {
        ...
        if (syncBtn) {
            if (currentSyncMode === 'normal') {
                ...
                if (!syncBtn.classList.contains('d-flex')) syncBtn.classList.add('d-flex');
        ...
        const radio = document.querySelector(`input[name="syncMode"][value="${currentSyncMode}"]`);
    ...
    document.getElementById('forcePullTriggerBtn')?.addEventListener('click', () => {
        ...
        if (currentSyncMode === 'strict_privacy') {
            showToast("Privacy Mode Enabled", "You must disable Strict Privacy Mode to pull data from the cloud.", "error");
            return;
        }
    ...
    document.getElementById('forcePushTriggerBtn')?.addEventListener('click', () => {
        // Guard: Block if in Strict Privacy Mode
        if (currentSyncMode === 'strict_privacy') {
            showToast("Privacy Mode Enabled", "You must disable Strict Privacy Mode to push data to the cloud.", "error");
            return;
        }
        $('#settingsModal').modal('hide');
